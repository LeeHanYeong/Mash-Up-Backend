...
class UserManager(BaseUserManager):
    def get_queryset(self):
        return (
            ...
            .annotate()
            .select_related()
            .prefetch_related("user_period_team_set",)
        )
...
class UserPeriodTeam(Model):
    is_active = models.BooleanField(
        "기수 활동여부", help_text="해당기수 중도 탈퇴시 False처리", default=True
    )
    user = models.ForeignKey(
        User,
        verbose_name="사용자",
        on_delete=models.CASCADE,
        related_name="user_period_team_set",
        related_query_name="user_period_team",
    )
