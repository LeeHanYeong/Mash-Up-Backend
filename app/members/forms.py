        ...
        if not user.username:
            ...
            if not email:
                ...
                while User.objects.filter(
                    Q(username=current_name) | Q(name=current_name)
                ).exists():
    ...
    def save(self, *args, **kwargs):
        ...
        if self.cleaned_data["is_staff"]:
            ...
            if group in self.cleaned_data["groups"]:
                self.cleaned_data["groups"] = self.cleaned_data["groups"].exclude(
                    id=group.id
                )
...
class LoginForm(forms.Form):
    name = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "이름을 입력해주세요",},
        ),
    )
    email = forms.CharField(
        required=False,
        widget=forms.EmailInput(
            attrs={"class": "form-control", "placeholder": "example@lhy.kr",},
        ),
    )
    password = forms.CharField(
        required=False,
        widget=forms.PasswordInput(
            attrs={"class": "form-control", "placeholder": "처음 로그인한다면 비워주세요",},
        ),
    )
    ...
    def clean(self):
        ...
        fields = ("name", "email")
        not_none_fields = {
            field: cleaned_data[field] for field in fields if bool(cleaned_data[field])
        }
        try:
...
class PasswordSetForm(forms.Form):
    ...
    password2 = forms.CharField(
        label="비밀번호 확인",
        widget=forms.PasswordInput(
            attrs={"class": "form-control", "placeholder": "위와 같은 비밀번호를 입력해주세요",},
        ),
    )
