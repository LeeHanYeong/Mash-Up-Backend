...
class AttendanceAPITest(APITestCase):
    def test_attendance_update(self):
        ...
        attendances = [
            baker.make(Attendance, user=user, notice=notice) for user in users
        ]
        ...
        response = self.client.patch(
            ...
            data={"vote": Attendance.VOTE_ATTEND,},
        )
        ...
        response = self.client.patch(
            ...
            data={"notice_id": attendance.notice.id, "vote": Attendance.VOTE_LATE,},
        )
        self.assertEqual(response.status_code, 200)
