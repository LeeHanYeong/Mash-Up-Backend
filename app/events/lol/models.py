...
class LOLTeam(Model):
    ...
    def __str__(self):
        return "{leader}팀{members}".format(
            ...
            members=f' ({", ".join(self.player_set.values_list("name", flat=True))})'
            if self.player_set.exists()
            else "",
        )
...
class Player(TimeStampedModel, Model):
    ...
    def __str__(self):
        positions = ", ".join(
            [position.get_name_display() for position in self.position_set.all()]
        )
        return f"{self.name} (" f"{self.get_tier_display()} | " f"{positions})"
...
class PlayerPoint(Model):
    ...
    owner = models.ForeignKey(
        Player, on_delete=models.CASCADE, related_name="owner_point_set"
    )
    player = models.ForeignKey(
        Player, on_delete=models.CASCADE, related_name="point_set"
    )
    point = models.PositiveSmallIntegerField(
        choices=CHOICES_POINT, blank=True, null=True
    )
